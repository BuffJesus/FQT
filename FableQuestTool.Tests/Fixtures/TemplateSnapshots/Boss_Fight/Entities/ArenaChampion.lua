-- ArenaChampion.lua
-- Entity script for MyBossQuest
-- Generated by FSE Quest Creator Pro
-- Generator: FQT-StartScreenFix-2026-02-10

local Quest = nil
local Me = nil

function Init(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("ArenaChampion: Init called")
    Me:MakeBehavioral()
    Me:AcquireControl()
end

function Main(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("ArenaChampion: Main started")
    local hero = Quest:GetHero()

    -- Main behavior loop
    while true do
        if Quest:IsDistanceBetweenThingsUnder(hero, Me, 10.0) then
            Quest:AddScreenTitleMessage("The Champion Approaches", 3.0, true)
            Quest:EntitySetThingAsEnemyOfThing(Me, hero)
        end
        if Me:IsKilledByHero() then
            Quest:SetStateBool("bossDefeated", true)
            Quest:AddScreenTitleMessage("VICTORY", 3.0, true)
            Quest:SetStateBool("QuestCompleted", true)
            break
        end

        if Me:IsNull() then break end
        if not Quest:NewScriptFrame(Me) then break end
    end

    Me:ReleaseControl()
end
