-- VarBoolNPC.lua
-- Entity script for VarBoolQuest
-- Generated by FSE Quest Creator Pro
-- Generator: FQT-StartScreenFix-2026-02-10

local Quest = nil
local Me = nil

function Init(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("VarBoolNPC: Init called")
    Me:MakeBehavioral()
    Me:AcquireControl()
end

function Main(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("VarBoolNPC: Main started")
    local hero = Quest:GetHero()

    -- Main behavior loop
    while true do
        if Me:IsTalkedToByHero() then
            var_IsReady = true
                if var_IsReady then
                    Me:SpeakAndWait("Ready to go.")
                else
                    Me:SpeakAndWait("Not ready yet.")
                end
                if var_IsReady then
                    Me:SpeakAndWait("Ready to go.")
                else
                    Me:SpeakAndWait("Not ready yet.")
                end
        end

        if Me:IsNull() then break end
        if not Quest:NewScriptFrame(Me) then break end
    end

    Me:ReleaseControl()
end
