-- VarStringNPC.lua
-- Entity script for VarStringQuest
...
function Init(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("VarStringNPC: Init called")
    Me:MakeBehavioral()
    Me:AcquireControl()
end
...
function Main(questObject, meObject)
    Quest = questObject
    Me = meObject
    Quest:Log("VarStringNPC: Main started")
    local hero = Quest:GetHero()

    -- Main behavior loop
    while true do
        if Me:IsTalkedToByHero() then
            var_Greeting = "Nice to meet you."
                Me:SpeakAndWait(var_Greeting)
        end
