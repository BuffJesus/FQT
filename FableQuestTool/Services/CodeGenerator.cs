...
public sealed class CodeGenerator
{
    ...
    public string GenerateQuestScript(QuestProject quest)
    {
        ...
        sb.AppendLine($"-- Generator: {GeneratorStamp}");
        sb.AppendLine();
        ...
        bool hasStartScreenNode = quest.Entities.Any(entity =>
        ...
        if (quest.UseQuestStartScreen && !hasStartScreenNode)
        {
            GenerateStartScreenThread(sb, quest);
        }
    ...
    private void GenerateMainFunction(StringBuilder sb, QuestProject quest)
    {
        ...
        sb.AppendLine($"    Quest:Log(\"{GeneratorStamp}: generator active\")");
        sb.AppendLine();
        ...
        if (quest.UseQuestStartScreen && !hasStartScreenNode)
        {
            sb.AppendLine($"    Quest:CreateThread(\"FQT_StartScreen\", {{region=\"{primaryRegion}\"}})");
        }
    ...
    local pos_{entity.ScriptName} = nil
    if marker_{entity.ScriptName} ~= nil and marker_{entity.ScriptName}.GetPos ~= nil then
        local ok, result = pcall(function() return marker_{entity.ScriptName}:GetPos() end)
        if ok then
            pos_{entity.ScriptName} = result
        end
    end
    if pos_{entity.ScriptName} == nil then
    ...
    if pos_{entity.ScriptName} ~= nil then
        pcall(function() Quest:CreateCreature(""{entity.DefName}"", pos_{entity.ScriptName}, ""{entity.ScriptName}"") end)
    else
        Quest:Log(""FQT: Failed to resolve spawn position for {entity.ScriptName}"")
    end
    ...
    private void GenerateStartScreenThread(StringBuilder sb, QuestProject quest)
    {
        sb.AppendLine("function FQT_StartScreen(questObject)");
        sb.AppendLine("    Quest = questObject");
        sb.AppendLine("    local debugStart = Quest:GetStateBool(\"FQT_DebugStartScreen\")");
        sb.AppendLine("    local alreadyShown = Quest:GetStateBool(\"FQT_StartScreenShown\")");
        sb.AppendLine("    if alreadyShown == nil then alreadyShown = false end");
        sb.AppendLine("    if alreadyShown then");
        sb.AppendLine("        if debugStart then Quest:Log(\"FQT: start screen already shown\") end");
        sb.AppendLine("        return");
        sb.AppendLine("    end");
        sb.AppendLine("    Quest:Pause(0.5)");
        sb.AppendLine("    if not Quest:NewScriptFrame() then return end");
        sb.AppendLine($"    local questName = \"{quest.Name}\"");
        sb.AppendLine("    local activeQuest = Quest:GetActiveQuestName()");
        sb.AppendLine("    if debugStart then Quest:Log(\"FQT: active quest before start = \" .. tostring(activeQuest)) end");
        sb.AppendLine("    if activeQuest == nil or activeQuest == \"\" or activeQuest ~= questName then");
        sb.AppendLine("        Quest:ActivateQuest(questName)");
        sb.AppendLine("        Quest:Pause(0.1)");
        sb.AppendLine("        if not Quest:NewScriptFrame() then return end");
        sb.AppendLine("        activeQuest = Quest:GetActiveQuestName()");
        sb.AppendLine("        if debugStart then Quest:Log(\"FQT: active quest after activate = \" .. tostring(activeQuest)) end");
        sb.AppendLine("    end");
        sb.AppendLine("    Quest:SetHeroGuideShowsQuestCards(true)");
        if (!string.IsNullOrWhiteSpace(quest.QuestCardObject))
        {
    ...
    if pos_{entity.ScriptName} ~= nil then
        pcall(function() Quest:CreateObject(""{entity.DefName}"", pos_{entity.ScriptName}, ""{entity.ScriptName}"") end)
    else
        Quest:Log(""FQT: Failed to resolve spawn position for {entity.ScriptName}"")
    end
    ...
    private (int X, int Y) GetWorldMapOffset(QuestProject quest)
    {
        ...
        string primaryRegion = quest.Regions.FirstOrDefault() ?? string.Empty;
        if (string.IsNullOrWhiteSpace(primaryRegion))
        {
            primaryRegion = quest.ObjectiveRegion1;
        }

        if (string.IsNullOrWhiteSpace(primaryRegion))
        {
            primaryRegion = quest.ObjectiveRegion2;
        }

        return WorldMapCoordinateService.GetMapOffsetForQuest(primaryRegion ?? string.Empty, quest.Entities);
    }
